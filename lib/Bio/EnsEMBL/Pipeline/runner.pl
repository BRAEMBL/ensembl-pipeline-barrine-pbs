...
foreach my $id (@ARGV) {
    ...
    # If the job has a submission id already.
 #   my $submission_id_not_set = !$job->submission_id;
    
    ## When submitting, only the integer part is known.
    #my $submission_id_has_no_subjob_index = $job->submission_id !~ /\[/;
    
    #my $should_update_submission_id = $submission_id_not_set || $submission_id_has_no_subjob_index;
    
    #if($should_update_submission_id) {
    #  $submission_id_not_set = 1;
      ...
      if (!$PBS_JOBID) {
	$PBS_JOBID = 'Not set, probably local job.';
      }
      
      #$job->submission_id($ENV{'LSB_JOBID'});
      $job->submission_id($PBS_JOBID);
    #}
    #print "SETTING STATUS to WAITING\n";
    $job->set_status('WAITING');
    push(@jobs, $job);
}
#if($submission_id_not_set){
