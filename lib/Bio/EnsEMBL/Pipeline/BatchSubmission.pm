...
sub new {
  ...
  if ($batch_queue_entry)      { $self->batch_queue_entry($batch_queue_entry) }
...
sub batch_queue_entry{
   my ($self, $arg) = @_;

   if($arg){
     $self->{'batch_queue_entry'} = $arg;
   }

   return $self->{'batch_queue_entry'};
}
