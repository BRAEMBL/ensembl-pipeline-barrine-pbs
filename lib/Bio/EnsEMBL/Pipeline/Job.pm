...
sub flush_runs {
 ...
 ANAL:
  for my $anal (@analyses) {
    ...
    # Used to create separate job arrays for different analyses and retry 
    # statuses. The purpose is to only put pipeline jobs into an array that
    # have identical resrource requirements.
    #
                            -BATCH_QUEUE_ENTRY => $queue,
    );
	...
	# mnuhn: This is annoying, so commenting out.
    ...
    if ($@) {
      ...
      #logger_info("Created jobs for the following job_ids: @job_ids\n");
      print("Created jobs for the following job_ids: @job_ids\n");
...
sub set_up_queues {
  ...
  foreach my $queue (@$QUEUE_CONFIG) {
    ...
    # logic_name has to be copied over too so we can build meaningful 
    # directories for the sub arrays later.
    #
    $q{$ln}{logic_name} = $ln;
